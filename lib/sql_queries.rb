...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS amount_left_over FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING amount_left_over >= 0;"
end
...
"SELECT projects.category, pledge.amount FROM projects JOIN pledges ON projects.id = pledges.project_id WHERE projects.category = 'music'"
end
